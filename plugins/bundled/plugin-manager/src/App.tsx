        ...
        <PluginLayout title="插件管理">
            ...
            <Show when={view() === 'list'} fallback={<PluginDetailView />}>
                ...
                <div class="flex flex-col gap-2">
                    <For each={plugins()}>
                        {(plugin) => (
                            <Card onClick={() => setView({ pluginId: plugin.id })}>
                                <div class="flex items-center gap-3">
                                    ...
                                    <Switch
                                        ...
                                        onChange={(c) => handleToggle(plugin.id, c)}
                                    />
            ...
            <ConfirmDialog
                ...
                title="卸载插件"
                description={`确定要卸载 ${plugins()?.find(p => p.id === uninstallTarget())?.name ?? ''} 吗？此操作不可撤销。`}
                confirmLabel="卸载"
                variant="danger"
            />
        </PluginLayout>
                ...
                <section>
                    ...
                    <div class="flex flex-col gap-1">
                        <For each={plugin.commands}>
                            {(cmd) => (
                                <div class="flex items-center gap-2 py-1 px-2 rounded bg-surface-hover text-sm">
                                    <span>{cmd.title}</span>

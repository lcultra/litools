...
impl LitoolsApp {
    /// 同步构建 launcher 面板。search_results 由调用方异步获取后传入。
    pub fn launcher_panel_search_results(
        &self,
        query: &str,
    ...
    pub(crate) fn pinned_launcher_items(&self, limit: usize) -> LitoolsResult<Vec<LauncherItem>> {
        ...
        for record in pinned {
            if let Some(result) = result_for_target(
                &apps, &plugin_commands, &record.target_type, &record.target_id,
            )? {
                items.push(LauncherItem { result, is_pinned: true });
            }
        }
        Ok(items)
    }
...
pub(crate) fn section_if_not_empty(
    id: impl Into<String>, title: impl Into<String>, items: Vec<LauncherItem>,
) -> Option<LauncherSection> {
    if items.is_empty() { return None; }
    Some(LauncherSection { id: id.into(), title: title.into(), items })
}

pub(crate) fn result_for_target(
    apps: &AppRepository<'_>, plugin_commands: &PluginCommandRepository<'_>,
    target_type: &str, target_id: &str,
) -> LitoolsResult<Option<SearchResult>> {
    use litools_plugin::plugin_command_from_target_id;
    match target_type {
        "app" => Ok(apps.find_app(target_id)?.map(|app| search_result_for_app(app, ""))),
